...
\abstract{In recent years, memory has proven to be a constraining factor in many workloads. From embedded devices with a few megabytes of RAM to warehouse-scale computers with thousands of terabytes, RAM is an expensive necessity for all workloads. Memory compression has existed in all major operating systems for many years, but this compression, while faster than swapping to a disk, adds latency and pollutes caches. Companies and research groups have investigated hardware compression to mitigate these problems, but there do not exist open-source low-latency hardware compressors and decompressors, so everyone who studies hardware compression is forced to implement it themselves. Finally, because the devices that can benefit from memory compression vary so widely, there does not exist a single solution that can address the area, latency, power, and bandwidth requirements of all devices. This work is intended to address the many issues with hardware compressors and decompressors. This work implements three popular compression algorithms, LZ77, LZW, and Huffman encoding, as parameterized compressors and decompressors. The hardware is all open-source under a permissive license, meaning anyone and any organization is free to use or modify it as needed. Finally, the design is able to achieve decompression latencies under one microsecond and compression ratios comparable to software compression algorithms.}
...
\abstractgenaud{Computer memory, the fast, temporary storage where programs and data are held, is expensive and limited. Compression allows for data and programs to be held in memory in a format that allows for more data to be stored than would otherwise be possible by shortening data that is repeated multiple times. This work implements a hardware design for compression and decompression accelerators to make it faster for the programs using the compressed data to access it. This works includes three hardware compressor and decompressor designs that can be easily modified and are free for anyone to use however they would like.}
...
In all modern computing devices, from small IoT microcontrollers to warehouse-scale computers, memory is a valuable but limited resource. Memory compression is a technique that can enable significantly larger amounts of effective memory by representing data in a more compact format. This allows for less physical RAM to be used for a task, and it can improve performance in memory-bound applications by reducing the time spent swapping to a disk. Additionally, RAM requires a constant power draw to refresh its data, so using less physical RAM has the potential to lower power costs.

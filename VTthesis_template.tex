...
\acknowledge{I would like to thank my lovely wife, Emilie, for her support while I performed this research. I would also like to thank my family and friends for their support. I would like to thank Dr. Jian for his close work on my thesis. I also want to thank my other advisors for their advice and guidance throughout my academic career.}
...
This chapter covers important background information on how the compression algorithms relevant to this work function.

\section{Compression Algorithms}\label{se:compression_algorithms}
There are two primary types of lossless compression algorithms: those that alter character encoding methods and those that match sequences of characters. Although there are many examples of both compression techniques, this work focuses on Huffman encoding for character compression and LZ-based strategies for sequence compression. This is because they are the most common bases in the state-of-the-art for memory compression.
...
The first step in dynamic huffman encoding is to count and sort the frequency that each character occurs. The sorted frequencies of each example character occurrence are shown in Table \ref{tab:character_frequency_count}.
...
Now, the only task remaining is to encode each character using the codewords shown in Table \ref{tab:huffman_encodings}. The fully encoded example string of characters is shown below. Underscores are used to demonstrate separation between each character for the clarity of the reader. However, it is important to note that delimiters of any kind are unnecessary for Huffman encoding, because every character is a leaf node, so there is no way to mistake the meaning of a sequence of bits.

\[0\_0\_0\_10\_0\_0\_110\_10\_10\_0\_0\_10\_111\_0\_0\_10\]

As can be seen by the above resulting bit sequence, a significant number of bits have been saved. The original encoding would have required 32 bits, but this version only needs 25 bits. This simple example results in a compression ratio (calculated by dividing the resulting size by the initial size) of 1.28. This compression ratio and example does not include the overhead required to represent the Huffman tree, but this work uses a non-standard representation anyway, so these details are irrelevant to this simple example.
...
% TODO: it would be good to have a summary at the beginning of the literature review that summarizes why these existing works are inadequate
\section{}
...
% TODO: "This Contribution" is too generic for a section title, it needs to have a better name
...
% TODO: This reads like more of a background section. Cut this down significantly, and talk about how the design was implemented in Chisel, not just the benefits of Chisel
\subsection{Chisel}\label{ss:chisel}
...
% TODO: define what we mean by character frequency counting, maybe refer back to the background section here.
% TODO: define what we mean by encoding, to someone unfamiliar, the entire Huffman process is "encoding". Maybe call it "final character encoding?"
...
% TODO: for components that aren't in standard Huffman, give an example of the inputs to that module and what the outputs would be
% TODO: for components that are already in standard Huffman, reference when they happen in the examples given in the Background section
...
% TODO: go over this again with a specific audience in mind, maybe add references to previous sections in the paper for clarification
% TODO: things like "on the way down the list"
...
Each design shown is configured according to the typical configurations found in the design choices sections of Chapter \ref{ch:this_contribution}.
...
% TODO: update the LZ77 decompressor numbers with the real numbers from the Excel spreadsheet for an 8-way decompressor for LZ77
\begin{figure}[htb]
	\centering

...
Memory compression, the idea that portions of memory that are not in use can be compressed to take up less space, has been around for many years. Often, it works in a similar way to a normal swap device like a hard drive: RAM data that has not been recently used is stored somewhere else. However, instead of putting it on a disk, it is compressed, then stored back in RAM in a compressed form. Then, when it needs to be accessed, it can be decompressed and removed from the compressed virtual disk.
